...
> 使用场景: 对文章进行分类, 使用什么元素作为特征呢? 单词，字母，句子，段落 => 使用单词

使用 `CountVectorizer` 进行特征的提取

1. `CountVectorizer.fit_transform` 输入转换的文本， 输出转换之后词频的稀疏矩阵
2. `CountVectorizer.inverse_transform` 将转换之后的稀疏矩阵还原回转换之前的数据
